...
This vignette will show you how to use the `tidyprompt` package to
perform a sentiment analysis in R with a LLM (large language model).

We will first define a simple dataset of sentences that we will create
...
First we will create a connection to a locally running LLM (using
Ollama)
...
score. To do this, we will use ‘purrr’
[`map()`](https://purrr.tidyverse.org/reference/map.html) to iterate
...
to prompt the LLM for the sentiment score. The latter will force the LLM
...
![Plot of sentiment
scores](../reference/figures/plot_sentiment_analysis-1.png)
