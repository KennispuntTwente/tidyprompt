...
*To make it even easier to use 'tidyprompt', 
we are designing this package to be compatible with the ['ellmer'](https://ellmer.tidyverse.org/index.html) R package, which 
has gained popularity for interfacing with LLM APIs (see [more information below](#tidyprompt-versus-ellmer--tidyllm)).*
...
The `tidyprompt::llm_provider_ellmer()` function can create a LLM provider 
from an `ellmer::chat()` object, allowing users to use any LLM provider
that can be configured with 'ellmer', including the respective configuration
and features. Furthermore, `answer_as_json()` supports specifying a schema
as 'ellmer' structured data types (e.g., `ellmer::type_object()`).
When using an 'ellmer' LLM provider, `answer_as_json()` also calls
the native 'ellmer' chat function for obtaining structed output. 
(We will soon give `answer_using_tools()` similar support for defining tools with
`ellmer::tool()`.)
