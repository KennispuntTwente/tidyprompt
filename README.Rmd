...
(<koningluka@gmail.com>) and Tjark van de Merwe
...
with LLM APIs. Therefore, 'tidyprompt' now supports 'ellmer' chat objects as LLM providers. 
Users can pass an 'ellmer' chat object directly to `send_prompt()`, or use the `llm_provider_ellmer()`
function to create an LLM provider from an 'ellmer' chat object. This allows users to use any
LLM provider that can be configured with 'ellmer', including the respective 
...
and the two packages can be used together to elegantly design complex interactions with LLMs.
