...
object that interacts with the Ollama API.
...
- parameters:
  ...
  Additional parameters may be passed by adding them to the parameters
  list; these parameters will be passed to the Ollama API via the body
  of the POST request.

  Note that various Ollama options need to be set in a list named
  'options' within the parameters list (e.g., context window size is
  represented in \$parameters\$options\$num_ctx). For ease of
  configuration, the 'set_option' and 'set_options' functions are
  available (e.g., `$set_option("num_ctx", 1024)` or
  `$set_options(list(num_ctx = 1024, temperature = 1))`).
...
- url:
  ...
  The URL to the Ollama API endpoint for chat completion (typically:
  "http://localhost:11434/api/chat")

- num_ctx:

  The context window size to use. When NULL, the default context window
  size will be used. This is a function argument for convenience, and
  will be passed to '\$parameters\$options\$num_ctx'

## Value

A new
[llm_provider](https://kennispunttwente.github.io/tidyprompt/reference/llm_provider-class.md)
object for use of the Ollama API
