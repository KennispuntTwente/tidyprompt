...
When using `llm_provider_ellmer()` or a raw `ellmer::chat()` with `send_prompt()`,
'tidyprompt' rebuilds the working 'ellmer' turns from 'tidyprompt' chat history
for each call. That means the 'ellmer' chat mainly acts as provider
configuration plus native feature support here, while conversation state lives
in 'tidyprompt'. If you need the updated native 'ellmer' turns after a call,
use `send_prompt(..., return_mode = "full")` and inspect `$ellmer_chat`.
...
tools. Provider-specific 'ellmer' built-in tools are the exception: they only
work with 'ellmer'-backed providers. See example usage in the documentation of
`answer_using_tools()`.
